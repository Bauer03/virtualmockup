...
export class NoseHooverChain {
  ...
  constructor(params: NoseHooverChainParams) {
    ...
    this.Q[0] = (this.degreesOfFreedom / this.TEMP_TO_KE_FACTOR) * 
                this.targetTemperature * Math.pow(tau_T, 2) / (4 * Math.PI * Math.PI);
    
    for (let i = 1; i < this.chainLength; i++) {
      this.Q[i] = (1.0 / this.TEMP_TO_KE_FACTOR) * this.targetTemperature * 
                  Math.pow(tau_T, 2) / (4 * Math.PI * Math.PI);
    }
  }
  ...
  public applyThermostat(velocities: THREE.Vector3[], atomicMass: number, dt: number): void {
    ...
    const target_KE = (this.targetTemperature * this.degreesOfFreedom) / (2 * this.TEMP_TO_KE_FACTOR);
    ...
    for (let cycle = 0; cycle < this.numSubcycles; cycle++) {
      ...
      const G_M = (this.p_xi[this.chainLength - 2] * this.p_xi[this.chainLength - 2] / 
                   this.Q[this.chainLength - 2]) - target_KE / this.degreesOfFreedom;
  ...
  public calculateConservedQuantity(kineticEnergy: number, potentialEnergy: number): number {
    const target_KE_per_dof = this.targetTemperature / (2 * this.TEMP_TO_KE_FACTOR);
