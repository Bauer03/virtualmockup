...
export class MTTKBarostat {
  ...
  constructor(params: MTTKParameters) {
    ...
    for (let i = 0; i < params.pchain; i++) {
      ...
      const Q_i =
        (Math.pow(params.tau_T, 2) * params.temperature) /
        (4 * Math.PI * Math.PI);
  ...
  private calculateLongRangeCorrection(N: number, volume: number): number {
    ...
    const P_LRC =
      ((16 * Math.PI * rho * rho * epsilon * Math.pow(sigma, 3)) / 3) *
      ((2 * sr9) / 3 - sr3);
  ...
  updateBarostatThermostat(dt: number): void {
    ...
    const G_last =
      ((this.p_xi_barostat[M - 2] * this.p_xi_barostat[M - 2]) /
        this.Q_barostat[M - 2] -
        this.params.kB * this.params.temperature) /
      this.Q_barostat[M - 1];
    ...
    for (let i = M - 2; i >= 0; i--) {
      ...
      if (i === 0) {
        ...
        G_i =
          ((this.p_epsilon * this.p_epsilon) / this.W -
            this.params.kB * this.params.temperature) /
          this.Q_barostat[0];
      } else {
        // Other elements couple to previous chain element
        G_i =
          ((this.p_xi_barostat[i - 1] * this.p_xi_barostat[i - 1]) /
            this.Q_barostat[i - 1] -
            this.params.kB * this.params.temperature) /
          this.Q_barostat[i];
      }
  ...
  updateBarostatMomentum(P_inst: number, volume: number, dt: number): void {
    ...
    // Limit barostat acceleration to prevent instability
    const MAX_ACCELERATION = (0.01 * volume) / (dt * dt);
    const G_limited = Math.max(
      -MAX_ACCELERATION,
      Math.min(MAX_ACCELERATION, G_epsilon)
    );

    // SAFETY: Detect compression death spiral
    if (Math.abs(this.v_epsilon) > 0.1) {
      // Volume changing >10% per timestep
      console.warn(
        `[MTTK] Volume velocity too large: ${this.v_epsilon.toFixed(6)}`
      );
      this.v_epsilon *= 0.5; // Dampen aggressively
    }

    const xi_dot_0 = this.p_xi_barostat[0] / this.Q_barostat[0];
    const scale = Math.exp((-xi_dot_0 * dt) / 2);

    this.p_epsilon = scale * (scale * this.p_epsilon + (G_limited * dt) / 2); // Use limited
  ...
  getPressureStdDev(): number {
    ...
    const variance =
      this.pressureHistory.reduce((sum, p) => sum + Math.pow(p - mean, 2), 0) /
